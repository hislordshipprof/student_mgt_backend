...
const Tables = (props) => {
  ...
  const staffcolumns = [
    ...
    { field: "lastlogin",headerName: "Last Login",type: "date",width: 90,},
    {field: "datejoined",headerName: "Date Joined",type: "date",width: 90,},
  ];
  ...
  const baseUrl = "http://127.0.0.1:8000/api/manage_staff/";
  const baseUrl2 = "http://127.0.0.1:8000/api/manage_student/";
  const [mydata, setMyData] = useState([])
  const [mydatast, setMyDatast] = useState([])

const getData = () => {
  fetch(baseUrl)
    .then((response) => response.json())
    .then((mydata) => {
      console.log("m=dadas", mydata);
      setMyData(mydata);
...
const getDatast = () => {
  fetch(baseUrl2)
    .then((response) => response.json())
    .then((mydatast) => {
      console.log("my student data", mydatast);
      setMyDatast(mydatast);
